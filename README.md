...
> **This project has a [Wiki](https://zaney.org/wiki/zaneyos-2.2/). Find out how
> to use ZaneyOS here!** **I have put a lot of effort into the
> [documentation](https://zaney.org/wiki/zaneyos-2.2/) so it should be accurate.
...
- We are using the latest and greatest audio solution for Linux. If you like
  listening to music Spotify comes pre-installed out of the box. Not to mention
...
- A tmpfs for the /tmp directory is created improving rebuild time and reducing
  strain on hdd/ssd.
...
[read the wiki](https://zaney.org/wiki/zaneyos-2.2/).
...
- You can search the [Nix Packages](https://search.nixos.org/packages?) &
  ...
  you may face.
- To add a package there are the sections for it in config.nix and users.nix in
  your host folder. One is for programs available system wide and the other for
  your users environment only.
...
**🪧🪧🪧 Edit `hosts/<your-desired-hostname>/config.nix` and
...
called flake-rebuild that will rebuild the flake!
